...
- *ina219_Measurements*: This folder contains code for an Arduino Uno with an INA219 voltage and current sensor. All tests use this code for measuring. This code expects an Input in port 7, while this input is on HIGH it will do all measurements (Bus Voltage, Shunt Voltage, Load Voltage, Current) and print to the screen.
...
- *ESP_V3_LoRa_Idle*: Same setup from *ESP_V3_LoRa_Sender*, but this code only counts to a 100, with 100 ms delays. The objective of this code is to measure ESP32 energy consumption without LoRa transmission.
